    ...
    try {
      ...
      setMessage(
        data.event?.geoVerified
          ? "SE site visit recorded (face + geo verified)."
          : "SE site visit recorded (face verified; outside geofence)."
      );

      setVisitFaceFile(null);
      const input = document.getElementById("se-face-input") as HTMLInputElement;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-semibold">SE Dashboard – My Circle</h1>
        <Link href="/" className="text-xs px-2 py-1 border rounded">← Home</Link>
      </div>
      ...
      {projects.length > 0 && (
        <div className="border rounded p-3 space-y-3 text-sm">
          <div className="font-medium">Mark SE Site Visit (Face + GPS)</div>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <button
                ...
                className="bg-blue-600 text-white px-3 py-1 rounded text-xs mt-1 disabled:opacity-50"
              >
                {visitLoading ? "Marking…" : "Mark SE visit now"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ----------------------------- */}
      {/* PROJECT LIST WITH PROGRESS BARS + ATTENDANCE */}
      {/* ----------------------------- */}
      {projects.length > 0 && (
